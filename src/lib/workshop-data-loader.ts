...
export function getAvailableCustomLabor(model: string, workshopId: string) {
    ...
    // These services are handled in the "Recommended" section, so exclude them from the custom dropdown.
    const specialRecommendedServices = ["WHEEL ALIGNMENT (4 HEAD)", "WHEEL BALANCING - 4 WHEEL", "WHEEL BALANCING - 5 WHEEL", "STRUT GREASING"];
